...
```json
{
  ...
  "last_detection": {
    ...
    "count": 2,
    "boxes": [
      {"label": "bear", "conf": 0.86, "box": [233, 120, 412, 468], "height_frac": 0.41},
      {"label": "bear", "conf": 0.81, "box": [40, 215, 172, 512], "height_frac": 0.46}
    ]
  },
  "last_alert": {
...
- `last_detection.count` and `last_detection.boxes` describe detections meeting the configured height fraction during the latest streak.
...
- Page sections:
  1. **Header** with application title, status indicator (`<span id="status-indicator">`), and supporting tagline.
  2. **Live Panel** combining the MJPEG `<img id="video-stream">`, a detection overlay banner (`#detection-overlay`) that highlights the freshest YOLO hit, and a toolbar housing the monitoring class selector with manual controls.
  3. **Telemetry Panel** (`#metrics-panel`) presenting threshold settings, cooldown details, FPS values, and summaries for the most recent detection and alert.
  4. **Event Log** showing recent alerts as responsive cards with thumbnail preview, relative timestamp, and quick access link.
  5. **Debug Panel** toggled via button to reveal raw JSON from `status.debug` when enabled.
...
| `renderStatus(status)` | Update DOM for tracking class, telemetry metrics, cooldown text, and detection overlay. |
| `renderAlerts(alerts)` | Render alert cards with thumbnail, relative timestamp, and capture link. |
...
| `startPolling()` | Kick off the polling loop (via `setTimeout`) to refresh status/alerts. |
| `stopPolling()` | Clear the scheduled timeout (used on visibility change). |
