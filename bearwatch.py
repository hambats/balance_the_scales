        ...
        while not shutdown_event.is_set():
            ...
            for result in results:
                ...
                for idx, coords in enumerate(xyxy):
                    ...
                    if label.lower() == get_monitored().lower():
                        meets_threshold = height_frac >= ARGS.min_h_frac
                        target_hits.append(
                            {
                                "label": label,
                                "conf": conf,
            ...
            if saw_target:
                ...
                qualified_hits = [hit for hit in target_hits if hit.get("meets_threshold")]
                last_detection_info = {
                    "ts": _iso_now(),
                    "label": get_monitored(),
                    "conf": float(best_conf),
                    "count": len(qualified_hits),
                    "boxes": list(qualified_hits),
                }
                streak += 1
            else:
                streak = 0
...
def status() -> Response:
    ...
    info = last_detection_info
    if info:
        boxes = [
            {
                "label": hit.get("label"),
                "conf": hit.get("conf"),
                "box": list(hit.get("box", [])),
                "height_frac": hit.get("height_frac"),
            }
            for hit in info.get("boxes", [])
        ]
        detection_snapshot = {
            "ts": info.get("ts"),
            "label": info.get("label"),
            "conf": info.get("conf"),
            "count": info.get("count", len(boxes)),
            "boxes": boxes,
        }
    ...
    payload = {
        ...
        "last_detection": detection_snapshot,
